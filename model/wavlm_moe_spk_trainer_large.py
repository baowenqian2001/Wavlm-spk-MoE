...
class Moe(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, skip_size, num_experts=8, topk=4, noisy_gating=True, num_layers=2, bidirectional=True, dropout=0.0):
...
class speaker(nn.Module):
    def __init__(self):
        ...
        self.ASP = Attentive_Statistics_Pooling(dim=4096)
        ...
        self.speakerlayer1 = nn.Sequential(
            nn.Linear(2048, 256),
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Dropout(0.1)
    ...
    def forward(self, wavlm_feature):
        ...
        sp_e = sp_e.transpose(1,2).contiguous()
        # print(sp_e.shape,222)
        sp_e = self.ASP(sp_e)
        # print(sp_e,111)
        sp_e = self.speakerlayer(sp_e)
        speaker_embedding = self.speakerlayer1(sp_e)
        # speaker_embedding = self.classifier(speaker_embedding)
        # print(speaker_embedding.shape) #[12,2048]
        return speaker_embedding

class WavLM_ft(nn.Module):
    def __init__(self, model, model_path='/root/workspace/MDD/CTC-Attention-Mispronunciation/egs/wavlm/model/wavlm-large'):
...
class Wavlm_spk_Moe(nn.Module):
    def __init__(self, model_path, hidden_size=128, num_class=44, input_size = 128, spk_emb=256, experts_num=8, spk_num=7):
